...
def find_start_page(pdf_path: str, verbose: bool = False) -> int:
    ...
    1. Look for explicit section headings (References, Appendix, etc.)
    2. If not found, use pattern-based scoring to find reference-like pages
    ...
    # Stage 2: Find consecutive pages with high reference-likeness scores
    # Threshold: pages scoring above this are likely references
    THRESHOLD = 40.0
    CONSECUTIVE_PAGES = 2  # Need at least this many consecutive high-scoring pages
    
    for i in range(len(scores) - CONSECUTIVE_PAGES + 1):
        if all(s > THRESHOLD for s in scores[i:i+CONSECUTIVE_PAGES]):
            if verbose:
    ...
    # Stage 3: Fallback - return page with highest score
    if scores:
